...
| --- | --- | --- | --- |
| **Super App** | `my.alkhidmah.id` (+ iOS/Android) | Expo Router universal (`apps/native`) | Required (Better Auth) |
| **Public Landing** | `alkhidmah.id` | `apps/web` (TanStack Start, SSG-first) | None |
...
2. **`apps/native` sudah responsive** — SidebarTabBar untuk tablet/desktop, FloatingTabBar untuk mobile.
3. **Satu auth cookie domain** — cross-subdomain cookie (`sameSite: "none"`, `secure: true`). Login di `alkhidmah.id` berlaku di `my.alkhidmah.id`.
...
| **B** | Web app → Thin Landing | **HAF (Jan 2027)** | Strip `apps/web` jadi landing; redirect route interaktif ke `my.alkhidmah.id` |
| **C** | Ekhidmah store → Embedded | **Q2 2027** | Store jadi tab/domain di super app |
...
| **2027** | Functionality Coverage | Semua PRD domain MVP parity; ekhidmah embedded | Current team + 1 designer (hire Q4 2026) |
...
- **2027 H2**: Stream C (ekhidmah store embedding). Domain coverage lanjutan (store, organization). Designer onboard.
